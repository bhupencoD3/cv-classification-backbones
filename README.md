...
* **ResNet/**
  ...
  * `resnet50_transfer_learning.ipynb` → ResNet50 transfer learning using PyTorch on Fruits Classification Dataset (5 classes)
...
Each architecture has **two versions** (PyTorch and TensorFlow) and in some cases, additional **transfer learning** implementations.
...
### AlexNet Results
* **TensorFlow (20 epochs, Daisy vs Dandelion)** → **0.8102 accuracy**  
* **PyTorch (5 epochs, Daisy vs Dandelion)** → **82.42% accuracy**
...
### Inception Results
* **TensorFlow (Pretrained ImageNet)** → Top-5 predictions demonstrated  
* **PyTorch (Pretrained ImageNet)** → Top-5 predictions demonstrated  
...
### ResNet Results
* **TensorFlow (Pretrained ImageNet)** → Top-5 predictions demonstrated (e.g. *bustard: 99.99%*)  
* **PyTorch (Pretrained ImageNet)** → Top-5 predictions demonstrated  
* **Transfer Learning (Fruits Classification Dataset, 5 Classes)**  
  * **5 epochs** → **89% validation accuracy**  

📊 Screenshot:  
![ResNet50 PyTorch Transfer Learning Accuracy](screenshots/resnet_pytorch_accuracy.png)  
*ResNet50 – PyTorch (5 epochs, Fruits Classification Dataset – 89% validation accuracy)*
...
| ResNet50     | PyTorch    | Fruits Classification (TL)   | 5      | **89% validation accuracy** | ✅   |
